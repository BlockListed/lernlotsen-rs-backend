...
use crate::{
	auth::UserId,
	db::model::{TimeSlot, WebTimeSlot, DbTime, DbTimerange, self},
};
...
pub async fn get_timeslots(db: PgPool, u: UserId) -> anyhow::Result<Vec<WebTimeSlot>> {
	...
	let timeslots: Vec<WebTimeSlot> = timeslots_db.into_iter()
...
pub async fn get_timeslot_by_id(
	...
	Ok(Some(model::convert_ts(timeslot_db).context("invalid data in db")?))
}
...
pub async fn delete_timeslot_by_id(db: PgPool, u: UserId, id: Uuid) -> anyhow::Result<()> {
	sqlx::query!("DELETE FROM timeslots WHERE user_id = $1 AND id = $2", u.as_str(), id)
		.execute(&db)
		.await?;
